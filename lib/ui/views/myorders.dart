...
class _MyOrdersState extends State<MyOrders> {
  ...
  TextStyle unSelectedStyle = TextStyle(color: iconColor);
  TextStyle selectedStyle = TextStyle(color: Colors.white);

  @override
  Widget build(BuildContext context) {
    return Consumer<Userdata>(
      builder: (context, userData, child) => DefaultTabController(
        length: 5,
        child: Scaffold(
          ...
          body: Column(
            children: <Widget>[
              ...
              Flexible(
                  ...
                  child: TabBarView(
                    children: <Widget>[
                      All(orders: userData.myOrders),
                      Unpaid(orders: userData.unpaid),
                      Shipped(orders: userData.shipped),
                      ToBeShipped(orders: userData.toShip),
                      InDispute(orders: userData.issues)
                    ],
                  ))
            ],
          ),
...
class All extends StatelessWidget {
  ...
  const All({Key? key, required this.orders}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return OrderListDefaultPage(orders: orders);
  }
}
...
class Unpaid extends StatelessWidget {
  ...
  const Unpaid({Key? key, required this.orders}) : super(key: key);
  @override
  Widget build(BuildContext context) {
...
class Shipped extends StatelessWidget {
  ...
  const Shipped({Key? key, required this.orders}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return OrderListDefaultPage(
      orders: orders,
...
class ToBeShipped extends StatelessWidget {
  ...
  const ToBeShipped({Key? key, required this.orders}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return OrderListDefaultPage(
      orders: orders,
...
class InDispute extends StatelessWidget {
  ...
  const InDispute({Key? key, required this.orders}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return OrderListDefaultPage(
      orders: orders,
...
class OrderListDefaultPage extends StatefulWidget {
  ...
  const OrderListDefaultPage({Key? key, required this.orders})
      : super(key: key);
  @override
          ...
          child: CustomScrollView(
            slivers: <Widget>[
              _selectedView == View.LIST
                  ...
                  : SliverGrid.count(children: <Widget>[
                      for (var i = 0; i < widget.orders.length; i++)
                        Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: OrderItemGrid(order: widget.orders[i]))
                    ], crossAxisCount: 2)
            ],
          ),
        )
